...
if not file_found:
    raise ValueError('Cleaned dataset not found. Make sure to run create_data.py before spliting data.')
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=DATASPLIT[1] + DATASPLIT[2], random_state = SEED)
X_val, X_test, y_val, y_test = train_test_split(X_test, y_test, test_size=DATASPLIT[2] / (DATASPLIT[1] + DATASPLIT[2]), random_state = SEED)

X_train.to_csv(os.path.join(OUTPUT_PATH, 'training.csv'), index=False, header=True)
y_train.to_csv(os.path.join(OUTPUT_PATH, 'training_labels.csv'), index=False, header=True)

X_val.to_csv(os.path.join(OUTPUT_PATH, 'validation.csv'), index=False, header=True)
y_val.to_csv(os.path.join(OUTPUT_PATH, 'validation_labels.csv'), index=False, header=True)
